...
namespace UI
{
    public class PlayerStatsDisplay : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            _xpText.text =          $"XP : {Math.Round(PlayerController.Instance.Stats.LevelExperience, 1)} / {Math.Round(PlayerController.Instance.Stats.ExperienceToNextLevel, 1)}";
            _levelText.text =       $"Level : {PlayerController.Instance.Stats.Level}";
            _scoreText.text =       $"Score : {Mathf.RoundToInt(PlayerController.Instance.Stats.TotalExperience)}";
        }
    }
}